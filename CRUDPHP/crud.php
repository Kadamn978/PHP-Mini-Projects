...
if ($_POST['crudOperation'] == "saveData") {
    ...
    $editId = $_POST['editId'];
    $save = $obj->saveData($connection, $name, $email, $contact, $editId);
    if ($save) {
        echo "saved";
    }
}
...
if ($_POST['crudOperation'] == "getData") {
    $sr = 1;
    $tableData = '';
    $allData = $obj->getAllData($connection);
...
if ($_POST['crudOperation'] == "deleteData") {
    ...
    $delete = $obj->deleteData($connection, $deleteId);
    if ($delete) {
        echo "deleted";
    }
}
